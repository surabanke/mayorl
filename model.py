...
class Policy(nn.Module):
    def __init__(self, obs_shape, action_space, base_kwargs={}, curiosity=False, algo='A2C', model='MicropolisBase', args=None):
        ...
        # The environment is the source of truth for the tool count. Callers
        # (e.g. mayorl/train.py) pass the real value via base_kwargs['num_actions']
        # (= len(TOOLS) = 20, including the 'Nil' no-op tool). The old hardcoded
        # 19 dropped the 20th tool, so the policy could never sample a no-op.
        # Honour the caller's value for Micropolis; fall back to 19 only if none
        # was supplied. (Original root train.py passes 19, so it is unaffected.)
        caller_num_actions = base_kwargs.get('num_actions', None)
        if 'GameOfLife' in args.env_name:
            ...
            if args.power_puzzle:
                ...
                num_actions = caller_num_actions if caller_num_actions is not None else 19
