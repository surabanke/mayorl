...
#       engine.clearMap()
    ...
    def getTaxRate(self):
        """현재 세율 반환 (0~20 정수)."""
        return int(self.engine.cityTax)

    def setTaxRate(self, rate: int):
        """세율 설정 (0~20 정수). 높을수록 세수 많지만 인구 불만 증가."""
        self.engine.cityTax = max(0, min(20, int(rate)))

    def render_to_file(self, path: str, overlay: str = 'all'):
        """도시 맵을 PNG 파일로 저장. VLM 입력용."""
        surface = self.engine.getMapImage(overlay=overlay)
