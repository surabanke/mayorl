...
class ESOptimizer:
    def __init__(self,
       ...
       #logger.debug(engines)
        ...
        if is_candidate == False:
            ...
            logger.debug('Optimizer {} created!'.format(optim_id))
    ...
    def get_chunk(self, tasks):
        ...
        for task in tasks:
            gotten = task.get()
           #logger.debug('got {}'.format(gotten))
            l += 1
        return [gotten for task in tasks]
    ...
    def get_step(self, res, propose_with_adam=True, decay_noise=True, propose_only=False):
        logger.debug('getting optimizer step')
