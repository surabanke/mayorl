...
from poet_distributed.niches.micropolis.env import Env_config
from poet_distributed.reproduce_ops import Reproducer
from poet_distributed.novelty import compute_novelty_vs_archive
import json


def construct_niche_fns_from_env(args, env, seed, render=False):
    def niche_wrapper(configs, seed, render):  # force python to make a new lexical scope
        def make_niche():
            from poet_distributed.niches import CityNiche
            return CityNiche(env_configs=configs,
                    render=render,
                            seed=seed,
                            init=args.init,
    ...
    return niche_name, niche_wrapper(list(configs), seed, render)


class MultiESOptimizer:
    def __init__(self, args, engines, scheduler, client, r_engine):

        self.rendered_rank_0 = False
        self.args = args
    ...
    def create_optimizer(self, env, seed, created_at=0, model_params=None, is_candidate=False):
        ...
        assert env != None
        if not self.rendered_rank_0:
            render = True
        else:
            render = False
        render = True
        optim_id, niche_fn = construct_niche_fns_from_env(args=self.args, env=env, seed=seed, render=render)
